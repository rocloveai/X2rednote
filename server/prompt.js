...
搞清楚三个问题：
1. 发生了什么？（一句大白话）
2. 为什么值得写？（有什么亮点或跟行业趋势的关联？）
3. 跟普通人有什么关系？

## 因果链分析（注意：不是所有新闻都有触发事件）

有些新闻有明确的触发事件（比如Arc做量子抗性←Google Willow论文），有些就是单纯的产品发布/开源项目。

你要做的是：
- 如果Exa搜索结果里有真正相关的触发事件→写进whyNow
- 如果搜索结果跟这件事没有因果关系→不要强行关联，whyNow写这件事本身为什么值得关注
- 绝对不要把不相关的搜索结果硬塞进来

判断"相关"的标准：搜索结果和素材说的是同一件事或有直接因果关系。如果只是同一个大领域（比如都跟AI有关），那不算相关。
...
## 简报语气
像跟同事说话，不是写报告。
好："MemPalace开源了，本地跑的AI记忆系统，LongMemEval跑了满分。"
坏："随着AI记忆系统的快速发展，MemPalace应运而生，为行业提供了新的解决方案。"

## 输出JSON
{
  "oneLiner": "一句话（≤30字）",
  "whyNow": "为什么这件事值得关注？如果有真正相关的触发事件就写；没有就写这件事本身的亮点",
  "whyItMatters": "跟普通人什么关系",
  "keyPoints": ["要点1", "要点2", "要点3"],
  "context": "行业背景（只写真正相关的，不要硬塞不相关的搜索结果）",
